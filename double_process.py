...
'''
rough_histories = rough_histories[['Team','mu']]
rough_histories['conf'] = rough_histories.Team.map(conf_mapping).fillna('None')
rough_histories['conf_mean'] = rough_histories.groupby('conf').mu.transform('mean')

teambase = dict()
c = 15
all_teams = set(rough_histories.Team)
opponent_lists = list()
for teamname in all_teams:
    teambase[teamname] = elo_score(teamname, rough_histories[rough_histories.Team == teamname].conf_mean.iloc[0])
for teamname in all_teams:
    team_subset = df[df.Team == teamname]
    opponent_list = list()
    for _, row in team_subset.iterrows():
...
for clubname, opponent_list in list(zip(all_teams, opponent_lists)):
    teambase[clubname].update_naive(opponent_list)
pass2 = pd.concat([x.return_history() for x in teambase.values()])
pass2 = pass2[pass2.Date == "NAIVE"]
'''
